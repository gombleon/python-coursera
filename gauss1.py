def gauss(mat_a, mat_b):
    ...
    while column < len(mat_b):
        ...
        for row in range(column, len(mat_a)):
            if max_row is None or abs(mat_a[row][column]) > abs(
                    mat_a[max_row][column]):
                max_row = row
        if max_row is None:
            return None
        if max_row != column:
        ...
        for row in range(column + 1, len(mat_a)):
            mat_a[row] = [(a + k * (-mat_a[row][column])) for a, k in
                          zip(mat_a[row], mat_a[column])]
            mat_b[row] += -mat_b[column] * mat_a[row][column]
        column += 1
    solve_x = [0] * len(mat_b)
    for i in range(len(mat_b) - 1, -1, -1):
        solve_x[i] = (mat_b[i] - sum(
            x * a for x, a in zip(solve_x[(i + 1):], mat_a[i][(i + 1):])))
    print('OK')
    return solve_x
